...
export default function EditSignaturePage() {
  ...
  useEffect(() => {
    const fetchSignature = async () => {
      try {
        // We'll fetch signature data from the signatures list since we don't have a getById endpoint
        const data = await signatureService.getByInstitution();
        const signatureData = data.signatures.find((s: any) => s.id === params.id);
        ...
        if (signatureData) {
          ...
          if (signatureData.signature_url) {
            setSignaturePreview(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${signatureData.signature_url}`);
          }

          if (signatureData.stamp_url) {
            setStampPreview(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${signatureData.stamp_url}`);
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await signatureService.update(params.id as string, formDataToSend);
  ...
  if (fetchLoading) {
    return (
      <DashboardLayout>
        <LoadingSpinner text="Chargement de la signature..." />
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Modifier la signature</h1>
            <p className="mt-2 text-gray-600">
              Mettre à jour la signature numérique et le cachet
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="card space-y-6">
              ...
              {signaturePreview ? (
                <div className="mt-1 border border-gray-300 rounded-md p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-900">Signature actuelle:</span>
                    <button
                      type="button"
            ...
            <div>
              ...
              {stampPreview ? (
                <div className="mt-1 border border-gray-300 rounded-md p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-900">Cachet actuel:</span>
                    <button
                      type="button"
                      onClick={removeStamp}
                      className="text-red-600 hover:text-red-700 text-sm font-medium"
